...
public class SceneComponentAutoGen : MonoBehaviour
{
    ...
    public void GenerateNextFrame(int numberOfFrames, float frameLength)
    {
        // Duplicate this game object itself after frameLength pixels along x axis.
        nextFrame = Instantiate(this.gameObject, new Vector3(transform.position.x + frameLength, transform.position.y, transform.position.z), transform.rotation);

        if (numberOfFrames > 1)
        nextFrame.GetComponent<SceneComponentAutoGen>().GenerateNextFrame(numberOfFrames - 1, frameLength);
        //objectList.Add(nextFrame);
    }

    public void DestroySelf()
    {
        if (nextFrame != null)
            nextFrame.GetComponent<SceneComponentAutoGen>().DestroySelf();
